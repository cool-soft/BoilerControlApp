...
from backend.containers.services_containers.control_action_container \
    import ControlActionContainer
...
class FakeWeatherRepository(WeatherRepository):
    ...
    def get_weather_info(self, start_datetime: pd.Timestamp = None, end_datetime: pd.Timestamp = None) -> pd.DataFrame:
...
def calc_boiler_temp(boiler_temp_prediction_service, end_datetime, max_weather_temp, min_weather_temp, start_datetime,
    ...
    while current_temp <= max_weather_temp:
        ...
        boiler_temp_df = boiler_temp_prediction_service.predict_control_actions_async(start_datetime, end_datetime)
...
def main():
    ...
    temp_graph_container = TempGraphContainer(
    ...
    temp_requirements_container = TempRequirementsContainer(
        ...
        weather_service=providers.Singleton(FakeWeatherRepository)
    )

    boiler_temp_prediction_container = ControlActionContainer(
    ...
    boiler_temp_prediction_service = boiler_temp_prediction_container.temp_prediction_service()
