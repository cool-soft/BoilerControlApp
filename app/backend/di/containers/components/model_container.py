...
class ModelContainer(DeclarativeContainer):
    ...
    temp_correlation_table = Resource(heating_system_correlation_table, filepath=config.temp_correlation_table_path)
    timedelta_df = Resource(heating_objects_timedelta, filepath=config.timedelta_filepath)
    ...
    model_parameters = Factory(CorrTableModelParameters, timedelta_df)
